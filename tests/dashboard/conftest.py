"""Shared fixtures for tests/dashboard/ -- real bonbon_operator_api FastAPI
app + real auth, mirroring tests/production/conftest.py's fixtures
exactly (same real app/auth/permission code, not a re-implementation) so
the AI-model dashboard endpoints are tested through the same stack a real
operator dashboard request would hit."""
...
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "ros2_ws" / "src" / "bonbon_operator_api"))
...
def safety_gate(validator, aggregator, audit_logger):
    ...
    return SafetyCommandGate(validator=validator, status_aggregator=aggregator, audit_logger=audit_logger)


@pytest.fixture
def app(tmp_data_dir, auth_manager, role_manager, audit_logger, aggregator, mock_bridge, safety_gate):
...
def engineer_token(auth_manager):
    return _token_for(auth_manager, "test_engineer", "Engineer1234!", "engineer")
