...
class NavigationScenarioValidator:
    ...
    def validate(
        ...
        if metrics["navigation_success_rate"] < criteria.get(
            "min_navigation_success_rate", self.targets.min_navigation_success_rate
        ):
            return ValidationResult(False, "navigation success rate below target")
        if metrics["replanning_latency_ms"] > criteria.get(
            "max_replanning_latency_ms", self.targets.max_replanning_latency_ms
        ):
