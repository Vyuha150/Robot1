...
class PedestrianSimulator:
    ...
    def child_running(name: str, x: float, y: float) -> DynamicObstacle:
        return DynamicObstacle(
            name=name, kind="child_running", position=(x, y), velocity=(0.0, -2.2), radius_m=0.28
        )
    ...
    def wheelchair_user(name: str, x: float, y: float) -> DynamicObstacle:
        return DynamicObstacle(
            name=name, kind="wheelchair_user", position=(x, y), velocity=(0.35, 0.0), radius_m=0.55
        )

    @staticmethod
    def moving_cart(name: str, x: float, y: float) -> DynamicObstacle:
        return DynamicObstacle(
            name=name, kind="moving_cart", position=(x, y), velocity=(0.0, 0.65), radius_m=0.50
        )
