...
class BatteryRouter:
    ...
    def __init__(
        ...
        self._dock_goal_id:   Optional[str]   = None
        ...
        if is_charging and percentage >= self._cfg.resume_threshold_pct:
            if self._routing_active:
                logger.info(
                    ...
                    percentage, self._cfg.resume_threshold_pct,
                )
            self._routing_active = False
    ...
    def evaluate(
        ...
        if not self._cfg.enabled:
            return RoutingDecision(
                should_dock=False, level=self.classify(), charger=None,
                reason="battery routing disabled", urgency="normal",
            )
        ...
        if level == BatteryLevel.OK or level == BatteryLevel.CHARGING:
            return RoutingDecision(
                should_dock=False, level=level, charger=None,
                reason="battery adequate", urgency="normal",
            )
        ...
        if charger is None:
            ...
            return RoutingDecision(
                should_dock=False, level=level, charger=None,
                reason="no charger registered", urgency="normal",
            )
