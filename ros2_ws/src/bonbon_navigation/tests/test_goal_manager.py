...
def _enqueue(gm: GoalManager, goal_id: str = "", priority: int = 1,
             timeout_sec: float = 60.0,
             x: float = 1.0, y: float = 1.0, yaw: float = 0.0,
             preempt: bool = False) -> str:
    return gm.enqueue(
...
class TestTimeout:
    ...
    def test_timed_out_property_on_entry(self):
        entry = NavigationGoalEntry(
            goal_id="t1", goal_type=0,
            target_x=0, target_y=0, target_yaw=0, priority=1,
            timeout_sec=0.01, start_time=time.monotonic() - 1.0,
        )
        assert entry.timed_out is True

    def test_entry_not_timed_out_when_no_start(self):
        entry = NavigationGoalEntry(
            goal_id="t2", goal_type=0,
            target_x=0, target_y=0, target_yaw=0, priority=1,
            timeout_sec=0.01, start_time=None,
        )
...
class TestDistanceHelper:
    def test_distance_to_goal(self):
        entry = NavigationGoalEntry(
            goal_id="d1", goal_type=0,
            target_x=3.0, target_y=4.0, target_yaw=0, priority=1,
        )
    ...
    def test_distance_at_goal(self):
        entry = NavigationGoalEntry(
            goal_id="d1", goal_type=0,
            target_x=1.0, target_y=2.0, target_yaw=0, priority=1,
        )
