...
from bonbon_tts.config.tts_config import TTSConfig, PiperConfig, FillerConfig, QueueConfig, SpeakerConfig
        ...
        def on_activate(self, state: State) -> TransitionCallbackReturn:
            ...
            self._subs.append(
                self.create_subscription(
                    String, "/bonbon/tts/say",
                    self._cb_say, 10,
                )
            )
            self._subs.append(
                self.create_subscription(
                    String, "/bonbon/tts/say_priority",
                    self._cb_say_priority, 10,
                )
            )
            self._subs.append(
                self.create_subscription(
                    String, "/bonbon/tts/emergency",
            ...
            self._clear_srv = self.create_service(
                Empty, "/bonbon/tts/clear_queue", self._cb_clear_queue,
            )
            ...
            if cfg.speaker.driver == "hal":
                try:
                    ...
                    self.get_logger().warning(
                        "bonbon_hal not available; using MockSpeakerBridge"
                    )
