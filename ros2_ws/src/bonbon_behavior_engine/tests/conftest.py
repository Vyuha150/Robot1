"""Shared pytest fixtures + ROS2/message stubs for bonbon_behavior_engine
tests.

Why this exists
---------------
behavior_engine_node.py imports ``rclpy`` and the generated ``bonbon_msgs``
/ ``bonbon_srvs`` packages, which are not available in a plain (non-ROS2)
pytest environment. Every other test file in this package only exercises
the rclpy-free ``core/`` modules directly and never needed this -- but
verifying BehaviorEngineNode's own orchestration logic (_on_human_state /
_decide_multi_person_behavior: focus-person routing, snapshot management,
child-safety-modifier application, dispatch routing) means importing the
node class itself.

Adapted from bonbon_affective_ai/tests/conftest.py's proven stub pattern:
one complete, permissive set of stubs installed here (before any test
module is collected) so every test file in this directory sees the same
stubs, rather than each file racing to install its own incomplete set.

The message stub is permissive: any attribute can be set, and nested
message-like attributes (``msg.header.stamp``) auto-vivify, so it
tolerates every field the node sets without enumerating them.
"""
...
def _install_stubs() -> None:
    ...
    if "bonbon_srvs" not in sys.modules:
        ...
        for name in ("EvaluateCommand", "HealthCheck", "SetMode"):
